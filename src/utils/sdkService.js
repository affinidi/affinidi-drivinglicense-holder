import {
  AffinityWallet as Wallet
  // WalletStorageService
} from "@affinidi/wallet-browser-sdk"
...
class SdkService {
  ...
  async getCredentials(credentialShareRequestToken, fetchBackupCredentials) {
    const networkMember = await this.init()
    return networkMember.getCredentials(
      credentialShareRequestToken,
      fetchBackupCredentials
    )
  }
  ...
  async createCredentialShareResponseToken(
    ...
    const networkMember = await this.init()
    return networkMember.createCredentialShareResponseToken(
      credentialShareRequestToken,
      suppliedCredentials
    )
  }
